	...
	function upsertVestingPlans(
		...
		if (setupCount > 0) {
			vesting.setupVestingPlans(
				...
				_slice(toSetup, setupCount),
				_slice(toSetupAmounts, setupCount)
			);
		}

		if (resetCount > 0) {
			vesting.resetVestingPlans(
				token,
				_slice(toReset, resetCount),
				_slice(toResetAmounts, resetCount)
			);
		}
	}
