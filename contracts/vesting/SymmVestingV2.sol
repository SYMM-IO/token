...
contract SymmVestingV2 is VestingV2 {
	...
	function addLiquidity(
		...
		return _addLiquidityProcess(amount, minLpAmount, maxUsdcIn, planId);
	}
	...
	function addLiquidityByPercentage(
		...
		uint256 amount = (getTotalLockedAmount(msg.sender, SYMM) * percentage) / 1e18;
		return _addLiquidityProcess(amount, minLpAmount, maxUsdcIn, planId);
	}
	...
	function _addLiquidityProcess(
		...
		_claimUnlockedToken(SYMM, msg.sender, planId);

		VestingPlan storage symmVestingPlan = vestingPlans[SYMM][msg.sender][planId];
		...
		_claimUnlockedToken(SYMM_LP, msg.sender, planId);

		VestingPlan storage lpVestingPlan = vestingPlans[SYMM_LP][msg.sender][planId];
		...
		if (lpVestingPlan.isSetup()) {
			_resetVestingPlans(SYMM_LP, users, amounts, planIds);
		} else {
